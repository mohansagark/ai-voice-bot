    ...
    async fetch(request: Request, env: Env): Promise<Response> {
      ...
      if (url.pathname === "/health") {
        ...
        return Response.json(
          { ok: true, provider: config.defaultProvider, model: p?.model, tts: env.GOOGLE_TTS_API_KEY ? "google" : "browser", leads: env.DB ? "d1" : "none", mode: config.mode },
          { headers: cors },
        );
      }
      ...
      if (url.pathname === "/tts" && request.method === "POST") {
        ...
        const result = await synthesizeSpeech(body.text, voice, env.GOOGLE_TTS_API_KEY, deps.fetchImpl ?? fetch);
