...
const env = { GOOGLE_TTS_API_KEY: "x", ALLOWED_ORIGINS: "https://devmohan.in" } as unknown as Env;
...
describe("/tts", () => {
  it("returns audio bytes on a mocked Google TTS response", async () => {
    const fetchImpl = (async () => new Response(JSON.stringify({ audioContent: btoa("hey") }), { status: 200 })) as unknown as typeof fetch;
    const app = createApp({ ...stubDeps, fetchImpl });
    const res = await app.fetch(ttsReq({ text: "hi" }), env);
    expect(res.status).toBe(200);
    expect(res.headers.get("content-type")).toBe("audio/mpeg");
    expect(new TextDecoder().decode(await res.arrayBuffer())).toBe("hey");
  });
  ...
  it("returns 502 when GOOGLE_TTS_API_KEY is missing", async () => {
    const app = createApp({ ...stubDeps, fetchImpl: fetch });
    const res = await app.fetch(ttsReq({ text: "hi" }), { ALLOWED_ORIGINS: "https://devmohan.in" } as unknown as Env);
    expect(res.status).toBe(502);
  });

  it("forwards Google's failure status", async () => {
  ...
  it("forwards a 429 quota-exceeded status distinctly from other failures", async () => {
  ...
  it("bypasses origin + length guards in dev mode", async () => {
    ...
    const fetchImpl = (async () => new Response(JSON.stringify({ audioContent: btoa("x") }), { status: 200 })) as unknown as typeof fetch;
    const app = createApp({ ...stubDeps, fetchImpl });
    const res = await app.fetch(ttsReq({ text: "a".repeat(1300) }, "https://evil.example"), devEnv);
    expect(res.status).toBe(200);
  });

  it("/health reports the tts provider from GOOGLE_TTS_API_KEY presence", async () => {
    const app = createApp({ ...stubDeps, fetchImpl: fetch });
    const withKey = await app.fetch(new Request("https://w/health"), env);
    expect((await withKey.json() as any).tts).toBe("google");
    const noKeyEnv = { ALLOWED_ORIGINS: "https://devmohan.in" } as unknown as Env;
